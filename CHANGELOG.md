...
- `StatusCode` — `RawRepresentable<UInt16>` struct with RFC 6455 §7.4 close code constants (`normalClosure`, `goingAway`, etc.). Custom codes in the `4000`–`4999` private-use range are valid. See [wspulse/.github#37](https://github.com/wspulse/.github/issues/37).
