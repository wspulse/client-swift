...
- **`Sources/WspulseClient/Codec.swift`** — `WspulseCodec` protocol, `WireType` enum, `JSONCodec` default implementation.
- **`Sources/WspulseClient/Message.swift`** — `struct Message: Codable, Sendable` (`event`, `payload` — all optional).
